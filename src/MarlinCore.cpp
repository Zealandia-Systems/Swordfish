...
void event_probe_recover() {
	TERN_(HOST_PROMPT_SUPPORT, host_prompt(PROMPT_INFO, PSTR("G29 Retrying"), DISMISS_STR));
...
#	endif
	...
	SETUP_RUN(HAL_init());
	SETUP_RUN(Controller::getInstance().init());
...
#	endif
	...
	SET_OUTPUT(COOLANT_AIR_PIN);
	OUT_WRITE(COOLANT_MIST_PIN, COOLANT_MIST_INVERT); // Init Mist Coolant OFF
	OUT_WRITE(COOLANT_AIR_PIN, COOLANT_AIR_INVERT); // Init Mist Coolant OFF
#endif
