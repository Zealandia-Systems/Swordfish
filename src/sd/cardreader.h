...
#	if DISABLED(NO_SD_AUTOSTART) // Auto-Start auto#.g file handling
	...
	static inline void autofile_cancel() {
		autofile_index = 0;
	}
#	endif
	...
	static inline char* longest_filename() {
...
#	endif
	...
	static inline uint32_t getIndex() {
		return sdpos;
	}
	static inline uint32_t getFileSize() {
		return filesize;
	}
	...
	static inline int16_t read(void* buf, uint16_t nbyte) {
...
#			define SORTED_LONGNAME_MAXLEN  (SDSORT_CACHE_VFATS) * (LONG_FILENAME_LENGTH)
