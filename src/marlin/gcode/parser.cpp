...
std::string_view GCodeParser::selector_string, GCodeParser::parameter_string, GCodeParser::id_string;
...
void GCodeParser::reset() {
	...
	selector_string = { nullptr, 0 };
	parameter_string = { nullptr, 0 };
	id_string = { nullptr, 0 };
...
#if ENABLED(GCODE_MOTION_MODES)
			if (letter == 'G' && (codenum <= TERN(ARC_SUPPORT, 3, 1) || codenum == 5 || TERN0(G38_PROBE_TARGET, codenum == 38))) {
...
#	if ENABLED(ARC_SUPPORT)
		case 'I' ... 'J':
		case 'R':
			if (motion_mode_codenum != 2 && motion_mode_codenum != 3)
				return;
#	endif
		case 'P' ... 'Q':
 ...
 */
	...
	while (const char param = uppercase(*p++)) { // Get the next parameter. A NUL ends the loop
		...
		if (param == '?' && is_command('M', 2000)) {
			char* const end = strchr(p, ' ');

			selector_string = { p, end ? end - p : strlen(p) };
		...
		if (param == '>' && is_command('M', 2000)) {
			...
			parameter_string = { p, end ? end - p : strlen(p) };
		...
		if (param == '#') {
			...
			id_string = { p, end ? end - p : strlen(p) };
