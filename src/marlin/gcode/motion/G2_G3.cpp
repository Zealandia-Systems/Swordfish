...
void plan_arc(
	...
	uint16_t min_segments = 0;

	// Angle of rotation between position and target from the circle center.
	float angular_travel;
	float abs_angular_travel;
	...
	if (NEAR(current_position[p_axis], cart[p_axis]) && NEAR(current_position[q_axis], cart[q_axis])) {
		...
		switch (((angular_travel < 0) << 1) | clockwise) {
			case 1:
				...
				break; // Positive but CW? Reverse direction.
			case 2:
				angular_travel += RADIANS(360);
				break; // Negative but CCW? Reverse direction.
		}

		abs_angular_travel = ABS(angular_travel);

		const float portion_of_circle = abs_angular_travel / RADIANS(360);
	...
	//const FeedRate scaled_feed_rate = MMS_SCALED(feedrate_mm_s);
	const FeedRate scaled_feed_rate = feedrate_mm_s;
...
#	elif ARC_SEGMENTS_PER_SEC
			_MAX(scaled_feed_rate * RECIPROCAL(ARC_SEGMENTS_PER_SEC), MM_PER_ARC_SEGMENT)
#	else
			MM_PER_ARC_SEGMENT
#	endif
	);
...
#	endif
		...
		if (!planner.buffer_line(raw, scaled_feed_rate, active_extruder, MachineState::FeedMove, 0
#	if ENABLED(SCARA_FEEDRATE_SCALING)
		                         ,
		                         inv_duration
#	endif
	...
	planner.buffer_line(raw, scaled_feed_rate, active_extruder, MachineState::FeedMove, 0
#	if ENABLED(SCARA_FEEDRATE_SCALING)
	                    ,
	                    inv_duration
#	endif
...
#	endif
		...
		if (parser.seenval('R')) {
			...
			if (r) {
				...
				if (p1 != p2) {
					const Vector2f32 d2 = (p2 - p1) * 0.5f; // XY vector to midpoint of move from current
