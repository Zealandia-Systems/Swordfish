 ...
 * M212 - Set software end stop minimums.
 * M213 - Set software end stop maximums
...
public:
	...
	static inline uint8_t relative_mode() {
		return axis_relative;
	}

	static inline void restore_relative_mode(uint8_t mode) {
		axis_relative = mode;
	}
	 ...
	 */
	enum WorkspacePlane : char { PLANE_XY,
		                           PLANE_ZX,
		                           PLANE_YZ };
	static WorkspacePlane workspace_plane;
#endif

#define MAX_COORDINATE_SYSTEMS 60
...
#if HAS_TOOL_OFFSET
#	define MAX_TOOL_OFFSETS 50
	static tool_record_t tool_table[MAX_TOOL_OFFSETS];
#endif

	static millis_t previous_move_ms, max_inactive_time, stepper_inactive_time;
	FORCE_INLINE static void reset_stepper_timeout(const millis_t ms = millis()) {
		previous_move_ms = ms;
	}
	...
	static void process_subcommands_now_P(const char* gcode);
	...
	static inline bool set_autoreport_paused(const bool) {
		return false;
	}
#endif
...
private:
	...
	static void G0_G1(const bool fast_move = false);
...
#	endif
	...
	static void set_work_offsets(const uint8_t p);
	static void report_wcs_record(const uint8_t index);
	static void report_tool_record(const uint8_t index);
#endif
...
#if ENABLED(I2C_POSITION_ENCODERS)
	FORCE_INLINE static void M860() {
		I2CPEM.M860();
	}
	FORCE_INLINE static void M861() {
		I2CPEM.M861();
	}
	FORCE_INLINE static void M862() {
		I2CPEM.M862();
	}
	FORCE_INLINE static void M863() {
		I2CPEM.M863();
	}
	FORCE_INLINE static void M864() {
		I2CPEM.M864();
	}
	FORCE_INLINE static void M865() {
		I2CPEM.M865();
	}
	FORCE_INLINE static void M866() {
		I2CPEM.M866();
	}
	FORCE_INLINE static void M867() {
		I2CPEM.M867();
	}
	FORCE_INLINE static void M868() {
		I2CPEM.M868();
	}
	FORCE_INLINE static void M869() {
		I2CPEM.M869();
	}
#endif
