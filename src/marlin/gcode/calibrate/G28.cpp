...
// Disable the leveling matrix before homing
	...
	if (doZ) {
		if (homing) {
			...
			do_blocking_move_to_z(home_dir(Axis::Z()) > 0 ? Z_MAX_POS : Z_MIN_POS);
		}
	}
	...
	if (z_homing_height && (doX || doY || TERN0(Z_SAFE_HOMING, doZ))) {
		...
		do_z_clearance(z_homing_height, axis_is_trusted(Axis::Z()), DISABLED(UNKNOWN_Z_NO_RAISE));
	}
	...
	if (doX || (doY && ENABLED(CODEPENDENT_XY_HOMING) && DISABLED(HOME_Y_BEFORE_X))) {
		...
		if (homing) {
			...
			do_blocking_move_to_x(home_dir(Axis::X()) > 0 ? X_MAX_POS : X_MIN_POS);
		}
	...
	if (DISABLED(HOME_Y_BEFORE_X) && doY) {
		if (homing) {
			...
			do_blocking_move_to_y(home_dir(Axis::Y()) > 0 ? Y_MAX_POS : Y_MIN_POS);
		}
	}
