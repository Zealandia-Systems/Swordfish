...
void GcodeSuite::M115() {
  ...
  SERIAL_ECHO("{\"firmware\":{\"name\":\"Swordfish\"");
	SERIAL_PRINTF(",\"version\":\"%s\"", DETAILED_BUILD_VERSION);
	...
	// Machine Geometry
	#if ENABLED(M115_GEOMETRY_REPORT)
	Vector3f dmin = { X_MIN_POS, Y_MIN_POS, Z_MIN_POS };
	Vector3f dmax = { X_MAX_POS, Y_MAX_POS, Z_MAX_POS };
	...
	auto lmin = motionModule.toLogical(dmin);
	auto lmax = motionModule.toLogical(dmax);
	auto wmin = motionModule.toLogical(cmin);
	auto wmax = motionModule.toLogical(cmax);
