...
#	endif
	...
	static xyza_long_t delta_error;
	static xyza_ulong_t advance_dividend;
...
public:
	...
	static xyza_long_t count_position;

private:
	// Current stepper motor directions (+1 or -1)
	static xyza_int8_t count_direction;
...
public:
	...
	static inline bool is_awake() {
		return STEPPER_ISR_ENABLED();
	}
	...
	FORCE_INLINE static void initiateLA() {
		nextAdvanceISR = 0;
	}
#endif
...
#endif
#if ENABLED(X_DUAL_ENDSTOPS)
	FORCE_INLINE static void set_x_lock(const bool state) {
		locked_X_motor = state;
	}
	FORCE_INLINE static void set_x2_lock(const bool state) {
		locked_X2_motor = state;
	}
#endif
#if ENABLED(Y_DUAL_ENDSTOPS)
	FORCE_INLINE static void set_y_lock(const bool state) {
		locked_Y_motor = state;
	}
	FORCE_INLINE static void set_y2_lock(const bool state) {
		locked_Y2_motor = state;
	}
#endif
#if EITHER(Z_MULTI_ENDSTOPS, Z_STEPPER_AUTO_ALIGN)
	FORCE_INLINE static void set_z1_lock(const bool state) {
		locked_Z_motor = state;
	}
	FORCE_INLINE static void set_z2_lock(const bool state) {
		locked_Z2_motor = state;
	}
#	if NUM_Z_STEPPER_DRIVERS >= 3
	FORCE_INLINE static void set_z3_lock(const bool state) {
		locked_Z3_motor = state;
	}
#		if NUM_Z_STEPPER_DRIVERS >= 4
	FORCE_INLINE static void set_z4_lock(const bool state) {
		locked_Z4_motor = state;
	}
#		endif
#	endif
	...
	FORCE_INLINE static uint32_t calc_timer_interval(uint32_t step_rate, uint8_t* loops) {
		...
		*loops = multistep;
		...
		if (step_rate >= (8 * 256)) { // higher step rate
			...
			timer = (uint16_t) pgm_read_word(table_address) - (((uint16_t) pgm_read_word(table_address + 2) * (uint8_t) (step_rate & 0x0007)) >> 3);
		}
