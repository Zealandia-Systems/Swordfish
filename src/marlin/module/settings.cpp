...
typedef struct SettingsDataStruct {
	...
	u32 max_acceleration_unit_per_s2[4];
	u32 min_segment_time_us;
	f32 axis_steps_per_unit[4];
	f32 max_feed_rate_unit_per_s[4];
	f32 acceleration;
	f32 retract_acceleration;
	f32 travel_acceleration;
	f32 min_feedrate_unit_per_s;
	f32 min_travel_feedrate_unit_per_s;
...
#	define MOTOR_CURRENT_COUNT 3
	...
	//
	xyz_pos_t coordinate_system[MAX_COORDINATE_SYSTEMS]; // G54-G59
...
//
	...
	// CNC_COORDINATE_SYSTEMS
	//
	tool_record_t tool_table[MAX_TOOL_OFFSETS];

	uint8_t pocket_count;
...
#	define EEPROM_START() \
		...
		int64_t eeprom_index = EEPROM_OFFSET
#	define EEPROM_FINISH()  persistentStore.access_finish()
#	define EEPROM_SKIP(VAR) (eeprom_index += sizeof(VAR))
#	define EEPROM_WRITE(VAR) \
		do { \
			persistentStore.write_data(eeprom_index, (uintptr_t) &VAR, sizeof(VAR), &working_crc); \
		} while (0)
#	define EEPROM_READ(VAR) \
		do { \
			persistentStore.read_data(eeprom_index, (uintptr_t) &VAR, sizeof(VAR), &working_crc, !validating); \
		} while (0)
#	define EEPROM_READ_ALWAYS(VAR) \
		do { \
			persistentStore.read_data(eeprom_index, (uintptr_t) &VAR, sizeof(VAR), &working_crc); \
		} while (0)
...
bool MarlinSettings::save() {
	...
	//
	{
		...
		for (auto i = 0; i < 4; i++) {
			tmp1[i] = planner.settings.max_acceleration_unit_per_s2[i];
			tmp2[i] = planner.settings.axis_steps_per_unit[i];
			tmp3[i] = planner.settings.max_feedrate_unit_per_s[i];
		}

		EEPROM_WRITE(tmp1); // max_acceleration_mm_per_s2
		EEPROM_WRITE(planner.settings.min_segment_time_us);
		EEPROM_WRITE(tmp2); // axis_steps_per_mm
		EEPROM_WRITE(tmp3); // max_feedrate_mm_s

		EEPROM_WRITE(planner.settings.acceleration);
		EEPROM_WRITE(planner.settings.retract_acceleration);
		EEPROM_WRITE(planner.settings.travel_acceleration);
		EEPROM_WRITE(planner.settings.min_feedrate_unit_per_s);
		EEPROM_WRITE(planner.settings.min_travel_feedrate_unit_per_s);
...
#	else
		const xy_pos_t bilinear_start { 0, 0 }, bilinear_grid_spacing { 0, 0 };
#	endif
...
#	endif
	...
	//
	{
		...
		tmc_stealth_enabled_t tmc_stealth_enabled = { false, false, false, false, false, false, false, false, false, false, false, false, false, false, false };
/*#	if AXIS_HAS_STEALTHCHOP(X)
...
#	else
		const xyz_float_t backlash_distance_mm { 0, 0, 0 };
...
#	endif
	...
	_FIELD_TEST(tool_table);

#	if DISABLED(HAS_TOOL_OFFSET)
	const tool_record_t tool_table[MAX_TOOL_OFFSETS] = { { 0 } };
#	endif
	EEPROM_WRITE(TERN(HAS_TOOL_OFFSET, gcode.tool_table, tool_table));

	_FIELD_TEST(pocket_count);
	uint8_t pocket_count = 0;
	EEPROM_WRITE(pocket_count);

	_FIELD_TEST(current_tool);
	uint8_t current_tool = 0;
	EEPROM_WRITE(current_tool);
...
#	if ENABLED(CONTROLLER_FAN_EDITABLE)
	const controllerFan_settings_t& cfs = controllerFan.settings;
#	else
	controllerFan_settings_t cfs = { 0, 0 };
#	endif
	EEPROM_READ(cfs);
}
...
{
	_FIELD_TEST(tool_table);
#	if ENABLED(HAS_TOOL_OFFSET)
	if (!validating) {
		for (int i = 0; i < MAX_TOOL_OFFSETS; i++) {
			gcode.tool_table[i].offset = { 0 };
			gcode.tool_table[i].pocket = -1;
		}
	}

	EEPROM_READ(gcode.tool_table);
#	else
	tool_record_t tool_table[MAX_TOOL_OFFSETS];
...
//
{
	...
	EEPROM_READ(pocket_count);

	// atc.pocket_count(pocket_count);
}
...
if (eeprom_error) {
	...
	DEBUG_ECHO_START();
	DEBUG_ECHO(version);
	DEBUG_ECHOLNPAIR(" stored settings retrieved (", (long int) (eeprom_index - (EEPROM_OFFSET)), " bytes; crc ", (uint32_t) working_crc, ")");
}
...
#	endif // EXTRUDERS && !NO_VOLUMETRICS
	...
	CONFIG_ECHO_START();
	SERIAL_ECHOLNPAIR_P(
			PSTR("  M203 X"), LINEAR_UNIT(MMS_TO_MMM(planner.settings.max_feedrate_unit_per_s[Axis::X()])), SP_Y_STR, LINEAR_UNIT(MMS_TO_MMM(planner.settings.max_feedrate_unit_per_s[Axis::Y()])), SP_Z_STR, LINEAR_UNIT(MMS_TO_MMM(planner.settings.max_feedrate_unit_per_s[Axis::Z()]))
#	if DISABLED(DISTINCT_E_FACTORS)
, SP_A_STR, parser.radial_value_to_degrees(planner.settings.max_feedrate_unit_per_s[Axis::A()])
#	endif
	);
...
#			if AXIS_HAS_STEALTHCHOP(Z3)
	if (stepperZ3.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("I2 Z"), true);
	}
#			endif

#			if AXIS_HAS_STEALTHCHOP(Z4)
	if (stepperZ4.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("I3 Z"), true);
	}
#			endif

#			if AXIS_HAS_STEALTHCHOP(E0)
	if (stepperE0.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T0 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E1)
	if (stepperE1.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T1 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E2)
	if (stepperE2.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T2 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E3)
	if (stepperE3.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T3 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E4)
	if (stepperE4.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T4 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E5)
	if (stepperE5.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T5 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E6)
	if (stepperE6.get_stored_stealthChop()) {
		say_M569(forReplay, PSTR("T6 E"), true);
	}
#			endif
#			if AXIS_HAS_STEALTHCHOP(E7)
	if (stepperE7.get_stored_stealthChop()) {
