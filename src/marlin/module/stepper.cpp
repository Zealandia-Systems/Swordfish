...
Vector6i32 Stepper::endstops_trigsteps { 0, 0, 0, 0, 0, 0 };
Vector6i32 Stepper::count_position { 0, 0, 0, 0, 0, 0 };
Vector6i8 Stepper::count_direction { 0, 0, 0, 0, 0, 0 };
...
#define B_APPLY_DIR(v, Q) C_DIR_WRITE(v)
#define B_APPLY_STEP(v, Q) C_STEP_WRITE(v)
...
// Take multiple steps per interrupt (For high speed moves)
	...
	USING_TIMED_PULSE();
#endif
	Vector6bool step_needed { false, false, false, false, false, false };
...
uint32_t Stepper::block_phase_isr() {
	...
	if (!current_block) {
		...
		if ((current_block = planner.get_current_block())) {
			...
			if (!!current_block->steps.x())
				SBI(axis_bits, Axis::X());
			if (!!current_block->steps.y())
				SBI(axis_bits, Axis::Y());
			if (!!current_block->steps.z())
				SBI(axis_bits, Axis::Z());
			if (!!current_block->steps.a())
				SBI(axis_bits, Axis::A());
			if (!!current_block->steps.b())
				SBI(axis_bits, Axis::B());
			if (!!current_block->steps.c())
				SBI(axis_bits, Axis::C());
...
#if HAS_A_STEP
	A_STEP_INIT();
	A_STEP_WRITE(INVERT_A_STEP_PIN);
#endif
#if HAS_B_STEP
	B_STEP_INIT();
	B_STEP_WRITE(INVERT_B_STEP_PIN);
#endif
#if HAS_C_STEP
	C_STEP_INIT();
	C_STEP_WRITE(INVERT_C_STEP_PIN);
#endif
 ...
 */
void Stepper::_set_position(const Vector6i32& position) {
...
void Stepper::report_a_position(const Vector6i32& pos) {
	SERIAL_ECHOPAIR_P(PSTR(STR_COUNT_X), pos.x(), SP_Y_LBL, pos.y(), SP_Z_LBL, pos.z(), SP_A_LBL, pos.a(), SP_B_LBL, pos.b(), SP_C_LBL, pos.c());
}
