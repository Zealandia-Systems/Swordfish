...
void PrintCounter::loadStats() {
	...
	uint8_t value = 0;
	persistentStore.access_start();
	persistentStore.read_data(address, (uintptr_t) &value, sizeof(uint8_t));
	if (value != 0x16)
		initStats();
	else
		persistentStore.read_data(address + sizeof(uint8_t), (uintptr_t) &data, sizeof(printStatistics));
	persistentStore.access_finish();
	loaded = true;
...
#		if SERVICE_INTERVAL_1 > 0
	if (data.nextService1 == 0)
		doBuzz = _service_warn(PSTR(" " SERVICE_NAME_1));
#		endif
#		if SERVICE_INTERVAL_2 > 0
	if (data.nextService2 == 0)
		doBuzz = _service_warn(PSTR(" " SERVICE_NAME_2));
#		endif
#		if SERVICE_INTERVAL_3 > 0
	if (data.nextService3 == 0)
		doBuzz = _service_warn(PSTR(" " SERVICE_NAME_3));
#		endif
#		if HAS_BUZZER && SERVICE_WARNING_BUZZES > 0
	if (doBuzz)
		for (int i = 0; i < SERVICE_WARNING_BUZZES; i++)
			BUZZ(200, 404);
#		else
	UNUSED(doBuzz);
#		endif
...
void PrintCounter::saveStats() {
	...
	persistentStore.access_start();
	persistentStore.write_data(address + sizeof(uint8_t), (uintptr_t) &data, sizeof(printStatistics));
	persistentStore.access_finish();
...
void PrintCounter::showStats() {
	...
	SERIAL_ECHOLNPAIR(
			...
			", Failed: ", data.totalPrints - data.finishedPrints - ((isRunning() || isPaused()) ? 1 : 0) // Remove 1 from failures with an active counter
	);
...
#		if SERVICE_INTERVAL_2 > 0
		case 2:
			data.nextService2 = SERVICE_INTERVAL_SEC_2;
#		endif
#		if SERVICE_INTERVAL_3 > 0
		case 3:
			data.nextService3 = SERVICE_INTERVAL_SEC_3;
#		endif
	}
	saveStats();
}
...
#		if SERVICE_INTERVAL_1 > 0
		case 1:
			return data.nextService1 == 0;
#		endif
#		if SERVICE_INTERVAL_2 > 0
		case 2:
			return data.nextService2 == 0;
#		endif
#		if SERVICE_INTERVAL_3 > 0
		case 3:
			return data.nextService3 == 0;
#		endif
		default:
			return false;
	}
