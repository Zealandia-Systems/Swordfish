...
FORCE_INLINE swordfish::motion::FeedRate homing_feedrate(const Axis a) {
	...
	return swordfish::motion::FeedRate::UnitsPerSecond(MMM_TO_MMS(v));
}
...
inline float home_bump_mm(const Axis axis) {
  static const swordfish::math::Vector6f32 home_bump_mm_P DEFS_PROGMEM = HOMING_BUMP_MM;
...
#elif HOTENDS
  constexpr xyz_pos_t hotend_offset[HOTENDS] = { { 0, 0, 0 } };
#else
  constexpr xyz_pos_t hotend_offset[1] = { { 0, 0, 0 } };
#endif
...
FORCE_INLINE void do_blocking_move_to_a(const swordfish::status::MachineState machine_state, const float &ra, const std::optional<swordfish::motion::FeedRate> &feed_rate = std::nullopt) {
	auto target = current_position;
