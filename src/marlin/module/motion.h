...
FORCE_INLINE feedRate_t homing_feedrate(const AxisEnum a) {
	...
	switch (a) {
		case X_AXIS:
			v = homing_feedrate_mm_m.x;
			break;
		case Y_AXIS:
			v = homing_feedrate_mm_m.y;
			break;
		case Z_AXIS:
		default:
			v = homing_feedrate_mm_m.z;
...
void do_blocking_move_to(const xyza_pos_t& raw, const feedRate_t& fr_mm_s = 0.0f);
...
FORCE_INLINE void do_blocking_move_to_xy(const xyza_pos_t& raw, const feedRate_t& fr_mm_s = 0.0f) {
	do_blocking_move_to_xy(xy_pos_t(raw), fr_mm_s);
}
...
FORCE_INLINE void do_blocking_move_to_xy_z(const xyza_pos_t& raw, const float& z, const feedRate_t& fr_mm_s = 0.0f) {
	do_blocking_move_to_xy_z(xy_pos_t(raw), z, fr_mm_s);
}
...
FORCE_INLINE bool axis_was_homed(const AxisEnum axis) {
	return TEST(axis_homed, axis);
}
FORCE_INLINE bool axis_is_trusted(const AxisEnum axis) {
	return TEST(axis_trusted, axis);
}
FORCE_INLINE bool axis_should_home(const AxisEnum axis) {
	return (axes_should_home() & _BV(axis)) != 0;
}
FORCE_INLINE bool no_axes_homed() {
	return !axis_homed;
}
FORCE_INLINE bool all_axes_homed() {
	return xyz_bits == (axis_homed & xyz_bits);
}
FORCE_INLINE bool homing_needed() {
	return !all_axes_homed();
}
FORCE_INLINE bool all_axes_trusted() {
	return xyz_bits == (axis_trusted & xyz_bits);
}
 ...
 * Workspace offsets
 */
#if HAS_HOME_OFFSET || HAS_POSITION_SHIFT || HAS_TOOL_OFFSET
#	if HAS_HOME_OFFSET
...
// FORCE_INLINE void toLogical(xyza_pos_t &raw) { swordfish::controller.motionManager().toLogical(raw); }
void toNative(xy_pos_t& raw);
void toNative(xyz_pos_t& raw);
// FORCE_INLINE void toNative(xyza_pos_t &raw)  { swordfish::controller.motionManager().toNative(raw); }
#else
...
FORCE_INLINE void toLogical(xyza_pos_t&) {
}
FORCE_INLINE void toNative(xy_pos_t&) {
}
FORCE_INLINE void toNative(xyz_pos_t&) {
}
FORCE_INLINE void toNative(xyza_pos_t&) {
}
#endif
...
inline bool position_is_reachable(const float& rx, const float& ry) {
	if (!WITHIN(ry, Y_MIN_POS - fslop, Y_MAX_POS + fslop))
		return false;
...
FORCE_INLINE int x_home_dir(const uint8_t extruder) {
...
FORCE_INLINE int x_home_dir(const uint8_t) {
	return home_dir(X_AXIS);
}
