 ...
 */
 Vector6f32 current_position { X_HOME_POS, Y_HOME_POS, Z_HOME_POS, 0, 0, 0 };
...
FeedRate feedrate_mm_s = FeedRate::UnitsPerSecond(MMM_TO_MMS(homing_feedrate_mm_m.x));
...
void _internal_move_to_destination(const MachineState machine_state, const std::optional<FeedRate>& feed_rate
	...
	const auto old_feedrate = feedrate_mm_s;
...
//
static FeedRate saved_feedrate_mm_s = FeedRate::UnitsPerSecond(0);
...
FeedRate scaled_feed_rate = feedrate_mm_s;
	...
	if (parser.feedrate_type == FeedRateType::UnitsPerSecond) {
	  //scaled_feed_rate = MMS_SCALED(feedrate_mm_s);

		debug()("unscaled feed rate: ", feedrate_mm_s.value());
		debug()("scaled feed rate: ", scaled_feed_rate.value());
	}
...
#define SHOULD_HOME(A) TERN(HOME_AFTER_DEACTIVATE, axis_is_trusted, axis_was_homed)(A)
	...
	if (TEST(axis_bits, Axis::A()) && SHOULD_HOME(Axis::A()))
		CBI(axis_bits, Axis::A());
	return axis_bits;
}
...
void reference_rotary_axis(const Axis axis) {
	if (axis.is_linear()) {
		return;
	}

	const float move_length = 360;
	const float bump = 3;
	const float rebump = bump * 2;

	debug()("approach");
	endstops.enable_a_home(true);
	do_homing_move(axis, move_length, homing_feedrate(axis), true);
	endstops.enable_a_home(false);
...
void reference_linear_axis(const Axis axis) {
