...
constexpr xyza_feedrate_t _mf = MANUAL_FEEDRATE,
...
typedef struct {
	uint32_t max_acceleration_mm_per_s2[XYZA_N], // (mm/s^2) M201 XYZE
			min_segment_time_us; // (µs) M205 B
	float axis_steps_per_mm[XYZA_N]; // (steps) M92 XYZE - Steps per millimeter
	feedRate_t max_feedrate_mm_s[XYZA_N]; // (mm/s) M203 XYZE - Max speeds
...
public:
	...
	static uint32_t max_acceleration_steps_per_s2[XYZA_N]; // (steps/s^2) Derived from mm_per_s2
	static float steps_to_mm[XYZA_N]; // Millimeters per step
...
#	endif
	...
	static TERN(HAS_LINEAR_E_JERK, xyz_pos_t, xyza_pos_t) max_jerk;
#endif
...
#	endif
	...
	static xyza_long_t position;

#if HAS_POSITION_FLOAT
	static xyza_pos_t position_float;
#endif

#if IS_KINEMATIC
	static xyza_pos_t position_cart;
#endif
...
private:
	 ...
	 */
	static xyza_float_t previous_speed;
...
public:
	...
	FORCE_INLINE static void skew(xyz_pos_t& raw) {
		skew(raw.x, raw.y, raw.z);
	}
	...
	FORCE_INLINE static void unskew(xyz_pos_t& raw) {
	...
	FORCE_INLINE static void unapply_modifiers(xyza_pos_t& pos, bool leveling = ENABLED(PLANNER_LEVELING)) {
	...
	FORCE_INLINE static uint8_t nonbusy_movesplanned() {
	...
	FORCE_INLINE static bool is_full() {
		return block_buffer_tail == next_block_index(block_buffer_head);
	}

	// Get count of movement slots free
	FORCE_INLINE static uint8_t moves_free() {
...
// SCARA AB axes are in degrees, not mm
#if IS_SCARA
	FORCE_INLINE static float get_axis_position_degrees(const AxisEnum axis) {
	 ...
	 */
	FORCE_INLINE static bool has_blocks_queued() {
...
private:
	 ...
	 */
	static constexpr uint8_t next_block_index(const uint8_t block_index) {
		return BLOCK_MOD(block_index + 1);
	}
	static constexpr uint8_t prev_block_index(const uint8_t block_index) {
		return BLOCK_MOD(block_index - 1);
	}
	...
	FORCE_INLINE static void normalize_junction_vector(xyza_float_t& vector) {
		float magnitude_sq = 0;
		LOOP_XYZA(idx)
		if (vector[idx])
			magnitude_sq += sq(vector[idx]);
		vector *= RSQRT(magnitude_sq);
	}

	FORCE_INLINE static float limit_value_by_axis_maximum(const float& max_value, xyza_float_t& unit_vec) {
