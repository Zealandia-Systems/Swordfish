...
bool load_filament(const float &slow_load_length/*=0*/, const float &fast_load_length/*=0*/, const float &purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (pause_for_user) {
    ...
    wait_for_user = true;    // LCD click or M108 will clear this
      ...
      const char tool = '0'
        ...
        #endif
      ;
      host_prompt(PROMPT_USER_CONTINUE, PSTR("Load Filament T"), tool, CONTINUE_STR);
    #endif
  ...
  if (fast_load_length) {
    ...
    TERN_(HOST_PROMPT_SUPPORT, host_prompt(PROMPT_USER_CONTINUE, PSTR("Filament Purging..."), CONTINUE_STR));
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  KEEPALIVE_STATE(PAUSED_FOR_USER);
  TERN_(HOST_PROMPT_SUPPORT, host_prompt(PROMPT_USER_CONTINUE, GET_TEXT(MSG_NOZZLE_PARKED), CONTINUE_STR));
  ...
  while (wait_for_user) {
    ...
    if (nozzle_timed_out) {
      ...
      TERN_(HOST_PROMPT_SUPPORT, host_prompt(PROMPT_USER_CONTINUE, GET_TEXT(MSG_HEATER_TIMEOUT), GET_TEXT(MSG_REHEAT)));

      TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired_P(GET_TEXT(MSG_HEATER_TIMEOUT)));

      wait_for_user_response(0, true); // Wait for LCD click or M108

      TERN_(HOST_PROMPT_SUPPORT, host_prompt(PROMPT_INFO, GET_TEXT(MSG_REHEATING)));
      ...
      HOTEND_LOOP() thermalManager.heater_idle[e].start(nozzle_timeout);
      TERN_(HOST_PROMPT_SUPPORT, host_prompt(PROMPT_USER_CONTINUE, PSTR("Reheat Done"), CONTINUE_STR));
