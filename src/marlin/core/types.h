...
struct XYZval {
  ...
  FI operator bool()                                   { return z || x || y; }
	FI operator Eigen::Vector3<T>()            { return { x, y, z }; }
  ...
  FI operator       XYZEval<T>()                 const { return { x, y, z, 0 }; }
  FI       T&   operator[](const int i)                { return pos[i]; }
  FI const T&   operator[](const int i)          const { return pos[i]; }
...
struct XYZEval {
  ...
  FI bool        operator==(const XYZval<T>  &rs)             { return x == rs.x && y == rs.y && z == rs.z; }
  FI bool        operator!=(const XYZval<T>  &rs)             { return !operator==(rs); }
  FI bool        operator==(const XYZEval<T>  &rs)       const { return x == rs.x && y == rs.y && z == rs.z && e == rs.e; }
  FI bool        operator!=(const XYZEval<T>  &rs)       const { return !operator==(rs); }
