...
#if !(defined(TARGET_LPC1768) || defined(__SAMD51__)) && ANY( \
    ENDSTOPPULLDOWNS, \
...
static_assert(hbm[Axis::X()] >= 0, "HOMING_BUMP_MM.X must be greater than or equal to 0.");
static_assert(hbm[Axis::Y()] >= 0, "HOMING_BUMP_MM.Y must be greater than or equal to 0.");
static_assert(hbm[Axis::Z()] >= 0, "HOMING_BUMP_MM.Z must be greater than or equal to 0.");
...
#elif INVALID_TMC_SPI(B)
  #error "An SPI driven TMC driver on B requires B_CS_PIN."
#elif INVALID_TMC_SPI(C)
  #error "An SPI driven TMC driver on C requires C_CS_PIN."
#endif
...
#elif INVALID_TMC_UART(B)
  #error "TMC2208 or TMC2209 on B requires B_HARDWARE_SERIAL or B_SERIAL_(RX|TX)_PIN."
#elif INVALID_TMC_UART(C)
  #error "TMC2208 or TMC2209 on C requires C_HARDWARE_SERIAL or C_SERIAL_(RX|TX)_PIN."
#endif
...
#elif AXIS_DRIVER_TYPE_B(TMC2209)
  INVALID_TMC_ADDRESS(B);
#elif AXIS_DRIVER_TYPE_C(TMC2209)
  INVALID_TMC_ADDRESS(C);
#endif
...
#define INVALID_TMC_MS(ST) static_assert(0, "Invalid " STRINGIFY(ST) "_MICROSTEPS. Valid values are 0, 2, 4, 8, 16, 32, 64, 128, and 256.")
  ...
  INVALID_TMC_MS(A)
#elif !TMC_MICROSTEP_IS_VALID(B)
  INVALID_TMC_MS(B)
#elif !TMC_MICROSTEP_IS_VALID(C)
  INVALID_TMC_MS(C)
#endif
...
#undef _TMC_MICROSTEP_IS_VALID
  ...
  #define X_ENDSTOP_INVERTING !(AXIS_DRIVER_TYPE(X,TMC2209) || AXIS_DRIVER_TYPE(X,TMC2660))
  #define Y_ENDSTOP_INVERTING !(AXIS_DRIVER_TYPE(Y,TMC2209) || AXIS_DRIVER_TYPE(Y,TMC2660))
  #define Z_ENDSTOP_INVERTING !(AXIS_DRIVER_TYPE(Z,TMC2209) || AXIS_DRIVER_TYPE(Z,TMC2660))
 ...
 */
  ...
  #define _PIN_CONFLICT(P) (PIN_EXISTS(P) && P##_PIN == SPINDLE_LASER_PWM_PIN)
  ...
  #elif !PIN_EXISTS(SPINDLE_LASER_ENA) && DISABLED(SPINDLE_SERVO) && DISABLED(SPINDLE_MODBUS)
    #error "(SPINDLE|LASER)_FEATURE requires SPINDLE_LASER_ENA_PIN, SPINDLE_SERVO, or SPINDLE_MODBUS to control the power."
  #elif ENABLED(SPINDLE_CHANGE_DIR) && !PIN_EXISTS(SPINDLE_DIR) && DISABLED(SPINDLE_MODBUS)
    #error "SPINDLE_DIR_PIN is required for SPINDLE_CHANGE_DIR when not using SPINDLE_MODBUS."
    ...
    #endif
  #elif ENABLED(SPINDLE_MODBUS)
		#ifndef SPINDLE_MODBUS_TYPE
      #error "SPINDLE_MODBUS_TYPE is required with SPINDLE_MODBUS."
    #elif !MODBUS_TYPE_IS(CHANGRONG_H100)
      #error "SPINDLE_MODBUS_TYPE must be CHANGRONG_H100."
    #endif
  #endif
  #undef _PIN_CONFLICT
#endif
