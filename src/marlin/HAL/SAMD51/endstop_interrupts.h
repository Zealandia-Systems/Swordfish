...
void setup_endstop_interrupts() {
  #define _ATTACH(P) attachInterrupt(P, endstop_ISR, CHANGE)
    ...
    _ATTACH(Y_MAX_PIN);
  #endif
	#if ENABLED(Y_DUAL_ENDSTOPS) && HAS_Y2_MAX
		#if !AVAILABLE_EILINE(Y2_MAX_PIN)
			#error "Y2_MAX_PIN has no EXTINT line available."
		#endif
		_ATTACH(Y2_MAX_PIN);
	#endif
  #if HAS_Y_MIN
    ...
    _ATTACH(Y_MIN_PIN);
  #endif
	#if ENABLED(Y_DUAL_ENDSTOPS) && HAS_Y_MIN
		#if !AVAILABLE_EILINE(Y2_MIN_PIN)
			#error "Y2_MIN_PIN has no EXTINT line available."
		#endif
		_ATTACH(Y2_MIN_PIN);
	#endif
  #if HAS_Z_MAX
  ...
  #endif
	#if HAS_TOOL_PROBE
		#if !AVAILABLE_EILINE(TOOL_PROBE_PIN)
			#error "TOOL_PROBE_PIN has no EXTINT line available."
		#endif
		_ATTACH(TOOL_PROBE_PIN);
	#endif
	#if HAS_WORK_PROBE
		#if !AVAILABLE_EILINE(WORK_PROBE_PIN)
			#error "WORK_PROBE_PIN has no EXTINT line available."
		#endif
		_ATTACH(WORK_PROBE_PIN);
	#endif
}
