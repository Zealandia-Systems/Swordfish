...
namespace swordfish::tools::drivers {
	...
	ChangzouH100DriverImpl::ControlState ChangzouH100DriverImpl::readControlState() const {
		...
		return readHoldingRegister(0x0210, ControlState::Unknown);
	}
	...
	State ChangzouH100DriverImpl::readState() const {
		...
		return (State)(
				((controlState & (uint8_t)ControlState::Running || controlState & (uint8_t)ControlState::InRunning) ? (uint8_t)State::Running : 0)
			| ((controlState & (uint8_t)ControlState::Reverse || controlState & (uint8_t)ControlState::InReverse) ? 0 : (uint8_t)State::Forward)
			| (controlState & (uint8_t)ControlState::InBraking ? (uint8_t)State::Braking : 0)
			| ((controlState & (uint8_t)ControlState::Jogging || controlState & (uint8_t)ControlState::InJogging) ? (uint8_t)State::Jogging : 0)
		);
	}
