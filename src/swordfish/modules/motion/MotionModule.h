...
namespace swordfish::motion {
	...
	private:
		...
		static core::ObjectField<core::Vector3> __homeOffsetField;
		static core::ObjectField<core::Vector3> __workOffsetField;
		static core::ObjectField<core::Vector3> __toolOffsetField;
	...
	public:
		...
		virtual const char* name() override { return "Motion Module"; }
		...
		FORCE_INLINE float32_t toLogical(AxisEnum axis, float32_t value) { return value + _offset(axis); }
		FORCE_INLINE float32_t toNative(AxisEnum axis, float32_t value) { return value - _offset(axis); }
		
		FORCE_INLINE void toLogical(xy_pos_t &raw)                 { raw.x += _offset(X); raw.y += _offset(Y); }
		FORCE_INLINE void toLogical(xyz_pos_t &raw)			           { raw.x += _offset(X); raw.y += _offset(Y); raw.z += _offset(Z); }
		FORCE_INLINE Eigen::Vector2f toLogical(Eigen::Vector2f& raw) { return { raw(X) += _offset(X), raw(Y) += _offset(Y) }; }
		FORCE_INLINE Eigen::Vector3f toLogical(Eigen::Vector3f& raw) { return { raw(X) += _offset(X), raw(Y) += _offset(Y), raw(Z) += _offset(Z) }; }
			
		FORCE_INLINE void toNative(xy_pos_t &raw)				           { raw.x -= _offset(X); raw.y -= _offset(Y); }
		FORCE_INLINE void toNative(xyz_pos_t &raw)			           { raw.x -= _offset(X); raw.y -= _offset(Y); raw.z -= _offset(Z); }
		FORCE_INLINE Eigen::Vector2f toNative(Eigen::Vector2f& raw)  { return { raw(X) -= _offset(X), raw(Y) -= _offset(Y) }; }
		FORCE_INLINE Eigen::Vector3f toNative(Eigen::Vector3f& raw)  { return { raw(X) -= _offset(X), raw(Y) -= _offset(Y), raw(Z) -= _offset(Z) }; }
