...
namespace swordfish::data {
	struct Pagination {
		...
		Pagination(int16_t page = -1, int16_t pageLength = -1) :
	...
	class Table : public core::ObjectList<TRecord>, public ITable {
	private:
		TTable& derived() {
			return *static_cast<TTable*>(this);
		}

	protected:
		Table(core::Object* parent) :
				core::ObjectList<TRecord>(parent) {
		}
	...
	public:
		...
		virtual ITable* asTable() override {
			return static_cast<ITable*>(this);
		}

		virtual TRecord* get(int16_t index) {
		...
		virtual TRecord* getChild(int16_t index) override {
			int16_t i = 0;
		...
		virtual void writeRecordJson(io::Writer& out, TRecord& record) {
		...
		virtual void remove(Record& record) override {
			derived().removeInternal(static_cast<TRecord&>(record));
		}

		void removeInternal(TRecord& record) {
			core::ObjectList<TRecord>::remove(record);
		}
		...
		virtual Record& createChild() override {
			return static_cast<Record&>(core::ObjectList<TRecord>::emplaceBack());
		}
	};
} // namespace swordfish::data