...
namespace swordfish::core {
	...
	protected:
		Field(const char* name) : _name(name) {
	...
	protected:
		ValueFieldBase(const char* name) : Field(name) {
	...
	public:
		ValueField(const char* name, uint32_t byteOffset, T defaultValue) : ValueFieldBase(name), _byteOffset(byteOffset), _defaultValue(defaultValue) {
		...
		T get(Pack& pack) {
			...
			const auto* f = (const T*)(pack._values.data() + _byteOffset);
		...
		virtual void set(Pack& pack, const T value) {
			...
			T* f = (T*)(pack._values.data() + _byteOffset);
	...
	public:
		ValidatedValueField(const char* name, uint32_t byteOffset, T defaultValue, void (*validate)(T, T)) : ValueField<T>(name, byteOffset, defaultValue), _validate(validate) {
	...
	public:
		ValueField(const char* name, uint32_t bitOffset, bool defaultValue) : ValueFieldBase(name), _bitOffset(bitOffset), _defaultValue(defaultValue) {
	...
	protected:
		...
		ObjectFieldBase(const char* name, const uint32_t index) : Field(name), _index(index) {
	...
	public:
		ObjectField(const char* name, const uint32_t index, std::function<T*(Object*)> creator = createT) : ObjectFieldBase(name, index), _creator(creator) {
	...
	public:
		StaticObjectField(const char* name, const uint32_t index, T* object) : ObjectFieldBase(name, index), _object(object) {
	...
	public:
		TransientFieldBase(const char* name) : Field(name) {
	...
	public:
		TransientField(const char* name, std::function<TValue(TObject&)> getter, std::function<void(TObject&,TValue)> setter) : TransientFieldBase(name), _getter(getter), _setter(setter) {
	...
	public:
		TransientField(const char* name, std::function<const char*(TObject&)> getter) : TransientFieldBase(name), _getter(getter) {
