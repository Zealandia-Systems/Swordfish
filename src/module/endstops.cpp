...
#if HAS_WORK_PROBE
xyz_bool_t Endstops::work_probe_enabled = { 0, 0, 0 };
#endif
...
#if HAS_TOOL_PROBE
#	if ENABLED(ENDSTOPPULLUP_TOOL_PROBE)
	SET_INPUT_PULLUP(TOOL_PROBE_PIN);
#	elif ENABLED(ENDSTOPPULLDOWN_TOOL_PROBE)
	SET_INPUT_PULLDOWN(TOOL_PROBE_PIN);
#	else
	SET_INPUT(TOOL_PROBE_PIN);
#	endif
#endif

#if HAS_WORK_PROBE
#	if ENABLED(ENDSTOPPULLUP_WORK_PROBE)
	SET_INPUT_PULLUP(WORK_PROBE_PIN);
#	elif ENABLED(ENDSTOPPULLDOWN_WORK_PROBE)
	SET_INPUT_PULLDOWN(WORK_PROBE_PIN);
#	else
	SET_INPUT(WORK_PROBE_PIN);
#	endif
#endif
...
void Endstops::resync() {
	if (!abort_enabled())
		return; // If endstops/probes are disabled the loop below can hang

	// Wait for Temperature ISR to run at least once (runs at 1KHz)
	TERN(ENDSTOP_INTERRUPTS_FEATURE, update(), safe_delay(2));
	while (TERN0(ENDSTOP_NOISE_THRESHOLD, endstop_poll_count))
		safe_delay(1);
}
...
void Endstops::run_monitor() {
	...
	monitor_count &= 0x7F;
	if (!monitor_count)
		monitor(); // report changes in endstop status
}
#endif

void Endstops::event_handler() {
	static uint8_t prev_hit_state; // = 0
	if (hit_state == prev_hit_state)
		return;
...
#define _ENDSTOP_HIT_TEST(A, C) \
	if (TEST32(hit_state, A##_MIN) || TEST32(hit_state, A##_MAX)) \
...
#	define P_AXIS Z_AXIS
		if (TEST32(hit_state, Z_MIN_PROBE))
...
#endif
#if HAS_TOOL_PROBE
	ES_REPORT(TOOL_PROBE);
#endif
#if HAS_WORK_PROBE
	ES_REPORT(WORK_PROBE);
#endif
#if HAS_FILAMENT_SENSOR
#	if NUM_RUNOUT_SENSORS == 1
	print_es_state(READ(FIL_RUNOUT1_PIN) != FIL_RUNOUT1_STATE, PSTR(STR_FILAMENT_RUNOUT_SENSOR));
#	else
#		define _CASE_RUNOUT(N) \
			case N: \
				pin = FIL_RUNOUT##N##_PIN; \
				state = FIL_RUNOUT##N##_STATE; \
				break;
...
#define UPDATE_ENDSTOP_BIT(AXIS, MINMAX)  SET_BIT_TO32(live_state, _ENDSTOP(AXIS, MINMAX), (READ(_ENDSTOP_PIN(AXIS, MINMAX)) != _ENDSTOP_INVERTING(AXIS, MINMAX)))
#define COPY_LIVE_STATE(SRC_BIT, DST_BIT) SET_BIT_TO32(live_state, DST_BIT, TEST32(live_state, SRC_BIT))
...
#if HAS_TOOL_PROBE
	SET_BIT_TO32(live_state, TOOL_PROBE, (READ(TOOL_PROBE_PIN) != TOOL_PROBE_ENDSTOP_INVERTING));
#endif

#if HAS_WORK_PROBE
	SET_BIT_TO32(live_state, WORK_PROBE, (READ(WORK_PROBE_PIN) != WORK_PROBE_ENDSTOP_INVERTING));
#endif
...
#define TEST_ENDSTOP(ENDSTOP)      (TEST32(state(), ENDSTOP))
...
#define PROCESS_CORE_ENDSTOP(A1, M1, A2, M2) \
	do { \
...
#define PROCESS_DUAL_ENDSTOP(A, MINMAX) \
	do { \
...
#define PROCESS_TRIPLE_ENDSTOP(A, MINMAX) \
	do { \
...
#define PROCESS_QUAD_ENDSTOP(A, MINMAX) \
	do { \
...
#else
#	define PROCESS_ENDSTOP_X(MINMAX) \
		if (X_##MINMAX##_TEST()) \
		PROCESS_ENDSTOP(X, MINMAX)
#endif
...
#		endif
		...
		SBI32(live_state, Z_ENDSTOP);
		hit = true;
	}
#	endif

	if (TERN0(ENDSTOP_INTERRUPTS_FEATURE, hit))
		update();
...
void Endstops::clear_endstop_state() {
	TERN_(X_SPI_SENSORLESS, CBI32(live_state, X_ENDSTOP));
	TERN_(Y_SPI_SENSORLESS, CBI32(live_state, Y_ENDSTOP));
	TERN_(Z_SPI_SENSORLESS, CBI32(live_state, Z_ENDSTOP));
}
...
void Endstops::monitor() {
	...
	static esbits_t old_live_state_local = 0;
	static uint8_t local_LED_status = 0;
	esbits_t live_state_local = 0;

#	define ES_GET_STATE(S) \
		if (READ(S##_PIN)) \
		SBI32(live_state_local, S)
...
#	define ES_REPORT_CHANGE(S) \
		if (TEST32(endstop_change, S)) \
		SERIAL_ECHOPAIR("  " STRINGIFY(S) ":", TEST32(live_state_local, S))
